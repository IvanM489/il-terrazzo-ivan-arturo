...
export default function Home() {
  ...
  const [overdueWatering, setOverdueWatering] = useState([]);
  const [wateringCheckDone, setWateringCheckDone] = useState(false);
  const [wateringAlertOpen, setWateringAlertOpen] = useState(false);
  ...
  useEffect(() => {
    async function checkWatering() {
      try {
        const response = await fetch("/api/plant-watering", { cache: "no-store" });
        ...
        const overdue = (result.records || []).filter((record) => {
          if (!record?.watered_at) return false;
          const wateredDate = new Date(record.watered_at);
          if (Number.isNaN(wateredDate.getTime())) return false;
          wateredDate.setHours(0, 0, 0, 0);
          const daysSinceWatering = Math.floor((today.getTime() - wateredDate.getTime()) / 86400000);
          return daysSinceWatering >= 5;
        }).map((record) => {
          const plant = (result.plants || []).find(
            (item) => String(item.id) === String(record.plant_id) && item.plantType === record.plant_type
          );
          const wateredDate = new Date(record.watered_at);
          wateredDate.setHours(0, 0, 0, 0);
          const daysSinceWatering = Math.floor((today.getTime() - wateredDate.getTime()) / 86400000);
          return {
            id: record.plant_type + ":" + record.plant_id,
            name: plant?.name || "Pianta",
            plantType: record.plant_type,
            daysSinceWatering,
          };
        });

        setOverdueWatering(overdue);
        setWateringAlertOpen(overdue.length > 0);
      } catch (error) {
        console.error("Errore controllo irrigazioni:", error);
      } finally {
        setWateringCheckDone(true);
      }
    }
    checkWatering();
  }, []);
  ...
  const overdueIndoor = overdueWatering.filter((plant) => plant.plantType === "indoor_plants");
  const overdueBonsai = overdueWatering.filter((plant) => plant.plantType === "bonsai");

  return (
    <>
      {(!wateringCheckDone || wateringAlertOpen) && (
        <div className="wateringAlertOverlay" role={wateringAlertOpen ? "alertdialog" : "status"} aria-modal={wateringAlertOpen ? "true" : undefined}>
          <div className="wateringAlertModal">
            {!wateringCheckDone ? (
              <>
                <div className="wateringAlertIcon">💧</div>
                <h2>Controllo irrigazioni</h2>
                <p>Controllo le ultime annaffiature di piante da interno e bonsai…</p>
              </>
            ) : (
              <>
                <div className="wateringAlertIcon">⚠️</div>
                <h2>Attenzione ‼️</h2>
                <p>Le seguenti piante sono state innaffiate <strong>5 o più giorni fa</strong>:</p>
                {overdueIndoor.length > 0 && (
                  <div className="wateringAlertGroup">
                    <h3>🪴 Piante da interno</h3>
                    <ul>
                      {overdueIndoor.map((plant) => (
                        <li key={plant.id}><strong>{plant.name}</strong><span>{plant.daysSinceWatering} giorni fa</span></li>
                      ))}
                    </ul>
                  </div>
                )}
                {overdueBonsai.length > 0 && (
                  <div className="wateringAlertGroup">
                    <h3>🌳 Bonsai</h3>
                    <ul>
                      {overdueBonsai.map((plant) => (
                        <li key={plant.id}><strong>{plant.name}</strong><span>{plant.daysSinceWatering} giorni fa</span></li>
                      ))}
                    </ul>
                  </div>
                )}
                <button className="wateringAlertButton" onClick={() => setWateringAlertOpen(false)}>OK, ho capito</button>
              </>
            )}
          </div>
        </div>
