...
function hmac(secret, value) { return crypto.createHmac("sha256", secret).update(value).digest("hex").toUpperCase(); }
...
async function getToken() {
  ...
  const { t, nonce, sign } = signRequest("GET", path, query);
  const response = await fetch(`${TUYA_BASE_URL}${path}?${query}`, {
    method: "GET",
    headers: { client_id: clientId, t, nonce, sign_method: "HMAC-SHA256", sign, "Content-Type": "application/json" },
    cache: "no-store",
  });
...
async function tuyaGet(path, query, token) {
  const clientId = process.env.TUYA_ACCESS_ID;
  const { t, nonce, sign } = signRequest("GET", path, query, token);
  const response = await fetch(`${TUYA_BASE_URL}${path}?${query}`, {
    method: "GET",
    headers: { client_id: clientId, access_token: token, t, nonce, sign_method: "HMAC-SHA256", sign, "Content-Type": "application/json" },
    cache: "no-store",
  });
...
export async function GET(request) {
  try {
    ...
    if (!result.success) return NextResponse.json({ error: result.msg || "Errore nello storico Tuya.", tuya: result }, { status: 500 });
    ...
    const events = logs
      ...
      .map((log) => {
        ...
        return { id: `${eventTime}-${log.code}`, date: Number.isFinite(eventTime) ? new Date(eventTime).toISOString() : null, eventTime, code: log.code, value: log.value === true || log.value === "true" };
      })
      .filter((event) => Number.isFinite(event.eventTime) && event.date !== null)
      .sort((a, b) => b.eventTime - a.eventTime);

    return NextResponse.json({ success: true, deviceId: DEVICE_ID, days, startTime, endTime, hasMore: result.result?.has_more === true, logCount: logs.length, events });
  } catch (error) {
